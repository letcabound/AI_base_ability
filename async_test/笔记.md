...
## 2. Fastapi 接口的 异步 和 background.add_task() 
事件循环：
    普通异步方法的 事件循环，是和主协程绑定的。但是Fastapi 服务接口应用的 事件循环 是和整个应用绑定的，和某个特定的路由接口无关。

Fastapi中的 background 机制:
    路由接口中，接收请求后调用 background.add_task() 将任务【注册】到事件循环，但只是注册并不执行，路由接口协程对象不会放开 cpu 控制权。
直到该接口 return返回HTTP响应，事件循环才会逐个处理其中的异步任务。background中的任务必须是异步任务，如果是阻塞任务则必须放在线程池，
否则会阻塞事件循环。如果fastapi服务退出，事件循环生命周期结束，background中的任务会被强制取消。


## 3. 异步迭代器和异步上下文管理
当要使用异步迭代器/上下文管理时，必须在一个定义好的 async def异步方法内使用。
异步迭代器：
    相较于迭代器，异步迭代器必须实现 __aiter__() 和 __anext__() 方法。

异步上下文管理：
    相较于上下文管理(with关键字)，异步上下文管理必须实现 __aenter__() 和 __aexit__() 方法。


## 4. uvloop
asyncio的事件循环的替代方案，其事件循环的运行效率优于 asyncio。
```
import asyncio
import uvloop
asyncio.set_event_loop_policy(uvloop.EventLoopPolicy()) # 用uvloop的事件循环管理替代默认的asyncio
